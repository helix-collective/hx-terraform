...
export function createApi(
  ...
  apiParams.resources.forEach(r => {
    ...
    const resource = AR.createApiGatewayResource(
      tfgen,
      name + '_' + r.name,
      params
    );
    r.methods.forEach(m => {
      ...
      const method = AR.createApiGatewayMethod(tfgen, mname, params);
      {
        ...
        const integration = AR.createApiGatewayIntegration(
          tfgen,
          mname,
          params
        );
      ...
      m.responses.forEach(mr => {
        ...
        const response = AR.createApiGatewayMethodResponse(
          tfgen,
          rname,
          params
        );
        tfgen.dependsOn(response, method);
        {
          ...
          const integration_response = AR.createApiGatewayIntegrationResponse(
            tfgen,
            rname,
            params
          );
...
export function lambdaPostMethod(
  region: AT.Region,
...
export function gatewayLambdaUri(
  region: AT.Region,
  lambda: AR.LambdaFunction
): string {
  return TF.rawExpr(
    `"arn:aws:apigateway:${region.value}:lambda:path/2015-03-31/functions/${
      lambda.arn.value
    }/invocations"`
  );
}
