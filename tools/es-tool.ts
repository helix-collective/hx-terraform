...
function splitIndex(
  index: string
): { pattern: string; date: string } | undefined {
  {
 ...
 */
function groupByDatedPattern(
  storageByIndex: StorageByIndex
): StorageByIndexPattern {
  ...
  for (const index of Object.keys(result.byDatedPattern)) {
    result.byDatedPattern[index].byDate.sort(
      (v1, v2) => (v1.date < v2.date ? -1 : v1.date > v2.date ? 1 : 0)
    );
  }
  return result;
}
...
async function printSummary(client: Client) {
  ...
  console.log(
    'IndexPattern         MinDate        MaxDate            NumDocs     SizeInBytes'
  );
  console.log(
    '------------------------------------------------------------------------------'
  );
  for (const pattern of Object.keys(groupedStorage.byDatedPattern)) {
    ...
    console.log(
      `${rpad(pattern, 20)} ${mindate}     ${maxdate} ${lpad(
        '' + s.total.docCount,
        15
      )} ${lpad('' + s.total.sizeInBytes, 15)}`
    );
  }
  console.log();
  ...
  for (const index of Object.keys(groupedStorage.other)) {
    ...
    console.log(
      `${rpad(index, 20)} ${lpad('' + s.docCount, 15)} ${lpad(
        '' + s.sizeInBytes,
        15
      )}`
    );
...
async function deleteOlderThanDate(
  client: Client,
  keepFrom: string,
  dryRun: boolean
) {
...
async function deleteOlderThanMonths(
  client: AwsClient,
  months: number,
  dryRun: boolean
) {
...
function usage() {
  ...
  console.log(
    'needs AWS_DEFAULT_REGION, AWS_ACCESS_KEY, AWS_SECRET_ACCESS_KEY to be set'
  );
  console.log();
}
