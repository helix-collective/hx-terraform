...
export function resourceIdAttr(name:string, params:RecordDecl): AttributeDecl {
  return {name, type:{kind: 'resourceid', resource:params.name}};
}

export interface Generator {
  generateResource(title, link, params: RecordDecl, attributes:AttributeDecl[]): void;
...
function camelFromSnake(s: string): string {
  return s.split('_').map( el => el[0].toUpperCase() + el.substr(1) ).join('');
}

function genType(type: Type): string {
  switch (type.kind) {
  case 'primitive': return type.type;
  case 'list': return genType(type.type) + "[]";
  case 'record': return paramsInterfaceName(type.name);
  case 'stringalias': return type.name;
  case 'resourceid': return type.name;
  case 'tagsmap': return 'TF.TagsMap';
  case 'enum': return type.values.map( name => `'${name}'` ).join(' | ');
  }
}
...
function genAttrTypeLabel(type: AttributeType): string {
  switch (type.kind) {
  case 'string': return type.typelabel;
  case 'resourceid': return camelFromSnake(type.resource) + 'Id';
  }
}
...
function genResourceFn(type: Type): string {
  switch (type.kind) {
  case 'primitive': return `TF.${type.type}Value`;
  case 'list': return `TF.listValue(${genResourceFn(type.type)})`;
  case 'record': return `(v) => TF.mapValue(fieldsFrom${paramsInterfaceName(type.name)}(v))`;
  case 'stringalias': return 'TF.stringAliasValue';
  case 'resourceid': return 'TF.resourceIdValue';
  case 'tagsmap': return 'TF.tagsValue';
  case 'enum': return 'TF.stringValue';
  }
}
...
export function fileGenerator(provider: string, headerLines: string[]) : FileGenerator {
  const lines: string[] = headerLines.concat([]);

  function generateResource(comment, link, params: RecordDecl, attributes: AttributeDecl[]) {
    ...
    lines.push(` *  see ${link}`);
    lines.push(` */`)
    lines.push(`export function create${name}(tfgen: TF.Generator, rname: string, params: ${paramsName}): ${name} {`)
  ...
  function generateParams(record: RecordDecl) {
    ...
    lines.push("}");
    lines.push("");
    lines.push(`export function fieldsFrom${interfaceName}(params: ${interfaceName}) : TF.ResourceFieldMap {`);
    lines.push("  const fields: TF.ResourceFieldMap = [];");
    for(const field of record.fields) {
      const fname = field.field;
      const toResourceFn = genResourceFn(field.type);
      if (field.optional) {
        lines.push(`  TF.addOptionalField(fields, "${fname}", params.${fname}, ${toResourceFn});`);
      } else {
        lines.push(`  TF.addField(fields, "${fname}", params.${fname}, ${toResourceFn});`);
      }
    }
