 ...
 */
export function createIamRolePolicy(tfgen:TF.Generator, role: AR.IamRole, policy:NamedPolicy): AR.IamRolePolicy {
 ...
 * Create an IAM Role.
 */
export function createIamRole(tfgen:TF.Generator, rname: string, assume_role_policy: string): AR.IamRole {
 ...
 */
export function createIamRoleWithPolicies(tfgen:TF.Generator, name: string, policies:NamedPolicy[]): AR.IamRole {
  const iamr = createIamRole(tfgen, name, JSON.stringify(assume_role_policy.policy, null, 2));
 ...
 */
export function createInstanceProfileWithPolicies(tfgen: TF.Generator, name: string, policies:NamedPolicy[]): AR.IamInstanceProfile {
