...
export function createPostgresInstance(
    ...
    const db_subnet_group = AR.createDbSubnetGroup(tfgen, name, {
      name: sname,
      subnet_ids: sr.network.azs.map(az => 
        params.use_external_subnets ? az.external_subnet.id : az.internal_subnet.id),
    });
  ...
  export function installMssqlTools() : bootscript.BootScript {
    ...
    bs.addAptRepository("deb [arch=amd64] https://packages.microsoft.com/ubuntu/16.04/prod xenial main");
