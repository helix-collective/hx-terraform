...
export function createEksCluster(
  ...
  return TF.withLocalNameScope(tfgen0, tfname, (tfgen) => {
    ...
    AR.createIamRolePolicyAttachment(tfgen, "clusterpolicy", {
      ...
      policy_arn: AT.arnT('arn:aws:iam::aws:policy/AmazonEKSClusterPolicy','IamPolicy'),
    });
    AR.createIamRolePolicyAttachment(tfgen, "servicepolicy", {
      role: cluster_role.name,
      policy_arn: AT.arnT('arn:aws:iam::aws:policy/AmazonEKSServicePolicy','IamPolicy'),
    });
    ...
    AR.createIamRolePolicyAttachment(tfgen, "workernode", {
      ...
      policy_arn: AT.arnT('arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy','IamPolicy'),
    });
    AR.createIamRolePolicyAttachment(tfgen, "workercni", {
      role: worker_role.name,
      policy_arn: AT.arnT('arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy','IamPolicy'),
    });
    AR.createIamRolePolicyAttachment(tfgen, "workercontaineregistryread", {
      role: worker_role.name,
      policy_arn: AT.arnT('arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly','IamPolicy'),
    });
...
#!/bin/bash
set -o xtrace
/etc/eks/bootstrap.sh --apiserver-endpoint '${cluster.endpoint}' --b64-cluster-ca '${certificate_authority_data}' '${params.cluster_name}'
`;
    ...
    const launch_configuration = AR.createLaunchConfiguration(tfgen, "worker", launch_configuration_params);
    tfgen.createBeforeDestroy(launch_configuration, true);

    const asg_name = tfgen.scopedName("asg").join("_");
    const asg_tags : AR.AutoscalingGroupTagParams[]  =  Object.entries(util.contextTagsWithName(tfgen,"asg")).map(
      ([key, value]) => {
        return {
          key,
          value,
...
interface EksClusterParams {
  ...
  node_launch_configuration_customize?: util.Customize<AR.LaunchConfigurationParams>,
